...
export default function FormRegisterNewFarm({ customer, closeDialog }: FormRegisterNewFarmProps) {
  ...
  const { mutate: registerNewFarm, isPending: isRegisteringNewFarm } = useRegisterNewFarm({
    onSuccess: () => {
      toast('Fazenda criada com sucesso');
      queryClient.invalidateQueries({ queryKey: ['farms'] });
      closeDialog();
    },
    onError: (error: Error) => {
      toast(`[FormRegisterNewFarm] ${error.message}`);
      console.error('[FormRegisterNewFarm] Error: ', error);
    },
  });

  const onSubmit = (data: z.infer<typeof NewFarmFormSchema>) => {
    if (!farmGeojson) {
          ...
          <Card className='h-full m-0 flex flex-col'>
            ...
            <div className='flex-shrink-0 p-6 pt-0'>
              <Button
                type='submit'
                className='w-full'
                disabled={!farmGeojson || isRegisteringNewFarm || convertionFileErrors.length > 0}
              >
                {isRegisteringNewFarm ? 'Criando fazenda...' : 'Criar fazenda'}
              </Button>
            </div>
          </Card>
                  ...
                  {farmGeojson ? (
                    <div className='space-y-1'>
                      ...
                      <div className='text-xs'>
                        ...
                        identificado{farmGeojson.length > 1 ? 's' : ''}
                      </div>
                    </div>
                  ) : (
