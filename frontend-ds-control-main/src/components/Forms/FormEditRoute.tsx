...
import { useUpdateRouteById } from '@/mutations/route.mutation';
import { useGetAllCustomers } from '@/queries/customer.query';
import { useGetAllFarms } from '@/queries/farm.query';
import { useGetRouteById } from '@/queries/route.query';
...
export default function FormEditRoute({ routeId, closeDialog }: FormEditRouteProps) {
  ...
  const { data: routeData, isLoading: isLoadingRouteData } = useGetRouteById(routeId, {
    includeFarm: 'true',
  ...
  const {
    ...
    setValue,
    watch,
  } = useForm<z.infer<typeof EditRouteFormSchema>>({
    resolver: zodResolver(EditRouteFormSchema),
    defaultValues: {
      // eslint-disable-next-line
      customerId: (routeData?.route as any)?.customer?.id,
      // eslint-disable-next-line
      farmId: (routeData?.route as any)?.farm?.id,
  ...
  useEffect(() => {
    ...
    setRouteGeoJson(routeData.route.geoJson);
    // eslint-disable-next-line
    setValue('customerId', (routeData.route as any).customer?.id);
    // eslint-disable-next-line
    setValue('farmId', (routeData.route as any).farm?.id);
    setValue('name', routeData.route.name);
  }, [routeData, setValue]);

  const { mutate: updateRoute, isPending: isUpdatingRoute } = useUpdateRouteById({
    onSuccess: () => {
      toast('Rota editada com sucesso');
      queryClient.invalidateQueries({ queryKey: ['routes'] });
      closeDialog();
    },
    onError: (error: Error) => {
      toast(`[FormEditRoute] ${error.message}`);
      console.error('[FormEditRoute] Error: ', error);
    },
  });

  const onSubmit = (data: z.infer<typeof EditRouteFormSchema>) => {
    if (!routeGeoJson) {
  ...
  if (isLoadingRouteData) {
    return (
      <div className='max-w-7xl h-[600px] p-6'>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 h-full gap-6 p-6'>
          <Card className='h-full m-0 flex flex-col'>
            <CardHeader className='flex-shrink-0'>
              ...
              <CardDescription>Edite os dados da rota</CardDescription>
            </CardHeader>
            ...
            <div className='flex-shrink-0 p-6 pt-0'>
              <Button
                type='submit'
                className='w-full'
                disabled={!routeGeoJson || isUpdatingRoute || convertionFileErrors.length > 0}
              >
                {isUpdatingRoute ? 'Salvando alterações...' : 'Salvar alterações'}
              </Button>
            </div>
          </Card>
                ...
                <div className='text-sm text-gray-600'>
                  {routeGeoJson ? (
                    <div className='space-y-1'>
                      ...
                      <div className='text-xs'>Rota pronta para ser editada</div>
                    </div>
                  ) : (
