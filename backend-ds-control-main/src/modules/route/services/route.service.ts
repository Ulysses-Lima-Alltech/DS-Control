...
import {
  RouteVM,
  type RouteViewModelSchema
} from '@models/route.vm';
import { app } from '@modules/app/app.module';
import { RouteRepository } from '@repositories/routes/route.repository';
import type { Route, RouteOrderBy, RouteOrderType, RouteWithCustomer, RouteWithFarm, RouteWithFarmAndCustomer } from '@repositories/routes/route.types';
...
export class RouteService {
  ...
  public async createRoute({
    ...
    farmId,
    customerId,
  }: CreateRouteDTO): Promise<void> {
    app.log.info(
      '[RouteService] - Starting route creation for farm %s',
      farmId,
    );
    ...
    app.log.info(
      '[RouteService] - Route created successfully with ID %s',
      route.id,
    );
  }
  ...
  public async listRoutes({
    ...
    orderBy,
    orderType,
  }: RouteSearchQueryString): Promise<
    PaginatedRequest<typeof RouteViewModelSchema>
  > {
    app.log.info('[RouteService] - Listing all routes');

    const queryResult =
      await this.routeRepository.getAllRoutesWithRelations(
        page,
        limit,
        search,
        customerId,
  ...
  public async getAllRoutes(
    ...
    if (routeId && customerId && queryResult.length === 0) {
      ...
      throw new AppError(
        'A rota não pertence a este cliente',
    ...
    if (customerId && queryResult.length === 0) {
      app.log.warn(
        '[RouteService] - Customer not found: %s',
        customerId,
      );
      throw new AppError(
    ...
    if (routeId && !customerId && !farmId && queryResult.length === 0) {
      app.log.warn(
        '[RouteService] - Route not found: %s',
        routeId,
      );
      throw new AppError(
        'Rota não encontrada',
  ...
  public async getRouteById(
    ...
    app.log.info(
      '[RouteService] - Fetching route details for route %s',
      routeId,
    );

    const route = await this.routeRepository.getRouteWithRelationsById(routeId, includeFarm, includeCustomer);
    ...
    app.log.info(
      '[RouteService] - Successfully retrieved route details for %s',
      routeId,
    );
    return route;
  }
  ...
  public async getRoutesByFarmId(farmId: string): Promise<Route[]> {
    app.log.info(
      '[RouteService] - Fetching routes for farm %s',
      farmId,
    );
  ...
  public async getRoutesByCustomerId(customerId: string): Promise<Route[]> {
    app.log.info(
      '[RouteService] - Fetching routes for customer %s',
      customerId,
    );
  ...
  public async updateRoute(
    ...
    app.log.info(
      '[RouteService] - Starting route update for route %s',
      routeId,
    );
    ...
    if (!existingRoute) {
      app.log.warn(
        '[RouteService] - Route update failed: Route %s not found',
        routeId,
      );
      throw new AppError(
        'Rota não encontrada',
    ...
    const updateData: { name?: string; geoJson?: Record<string, unknown>; farmId?: string; customerId?: string } = {};
    ...
    app.log.info(
      '[RouteService] - Route updated successfully with ID %s',
      routeId,
    );
    return updatedRoute;
  }
  ...
  public async deleteRoute(routeId: string): Promise<void> {
    app.log.info(
      '[RouteService] - Starting route deletion for route %s',
      routeId,
    );
    ...
    if (!existingRoute) {
      app.log.warn(
        '[RouteService] - Route deletion failed: Route %s not found',
        routeId,
      );
      throw new AppError(
        'Rota não encontrada',
    ...
    app.log.info(
      '[RouteService] - Route deleted successfully with ID %s',
      routeId,
    );
  }
}
