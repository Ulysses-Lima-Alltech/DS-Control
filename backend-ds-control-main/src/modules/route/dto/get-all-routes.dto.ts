...
export const GetAllRoutesQueryStringSchema = z.object({
  routeId: z.string().uuid().optional().describe('Route ID to filter routes by route'),
  ...
  includeFarm: z
    ...
    .describe('Include farm in the response'),
  includeCustomer: z
    .enum(['true', 'false'])
    .optional()
    .default('false')
    .transform((val) => val === 'true')
    .describe('Include customer in the response'),
  includeGeoJson: z
    .enum(['true', 'false'])
    .optional()
    .default('false')
  ...
  orderBy: z.nativeEnum(RouteOrderBy).optional().describe('Field to order the routes by'),
  orderType: z
